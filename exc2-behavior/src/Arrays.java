public class Arrays {
    ...
    /**
     * quickSort general d'alumnes
     * La manera com ordena els alumnes
     * està parametritzada
     */
    public static void quickSort(Student[] arr,
        ...
        int partition = partition(arr, start, end, comp);

        if (partition - 1 > start) {
            quickSort(arr, start, partition - 1, comp);
        }
        if (partition + 1 < end) {
            quickSort(arr, partition + 1, end, comp);
        }
    }

    /*
     * Mètode auxiliar de quickSort
     * La comparació entre estudiants
     * està determinada pel paràmetre
     * de tipus StudentComparator
     */
    public static int partition(Student[] arr,
                                int start, int end,
                                StudentComparator comp) {
        ...
        for (int i = start; i < end; i++) {
            ...
            //TODO: substituïu aquest true per la condició
            if (true) {
